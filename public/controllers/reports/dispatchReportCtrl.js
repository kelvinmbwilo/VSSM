...
//getting all regions
                ...
                $scope.filterDataStatus = function(vaccine_id,period){
                    ...
                    result.data = [];
                    var count = 0;
                    angular.forEach($scope.filterFilters($scope.filterTime( $scope.data.disItems,period)),function(val){
                ...
                $scope.prepareTitle = function(){
                    $scope.title = ""
                    $scope.title = (!$scope.data.recipient)?' ':"Recipient: "+$scope.getRecipientName($scope.data.recipient);
                    $scope.title = (!$scope.data.activity)?' ':" Activity: "+$scope.getActivityName($scope.data.activity);
                    $scope.title = (!$scope.data.store)?' ':" Store: "+$scope.getStoreName($scope.data.store);
                    $scope.title = (!$scope.data.sources)?' ':" Source: "+$scope.getSourceName($scope.data.sources);
