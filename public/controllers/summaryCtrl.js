...
//            xAxis: {currentMin: 0, currentMax: 10, minRange: 1},
        ...
        $scope.downloadData =function(voucher_number){
            ...
            $scope.newItem = {};
            $scope.newItem.items=[];
            angular.forEach($scope.sent_packages,function(value){
                if(value.package.voucher_number == voucher_number){
                    $scope.newItem.recipients = value.receiver_id;
                    $scope.newItem.transport_mode = value.package.transport_mode_id;
                    $scope.newItem.voucher_no = value.package.voucher_number;
                }
            });
...
// This can be changed to "pt" (points), "mm" (Default), "cm", "in"
            ...
            var i = 1;
            var total_price = 0;
            angular.forEach($scope.sent_packages,function(data){
                if(data.package.voucher_number == voucher_number){
                    total_price+= data.unit_price * data.amount;
                    datas.push(
                        {'sn':i,'item':data.vaccine.name,'batch_number':data.batch_number,'manufacture':$scope.getManufactureName(data.packaging.manufacture_id),'expire_date':data.expiry_date,'doses':data.amount,'vials':data.amount/data.packaging.dose_per_vial,'t_price':data.unit_price * data.amount}
                    );
                    i++;
                }
            });
