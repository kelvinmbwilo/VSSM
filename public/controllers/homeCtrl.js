    ...
    .controller("mainCtrl",function ($rootScope,$window,$scope,$http,$location,$timeout,$translate,DTOptionsBuilder,$filter) {
        ...
        $http.get("index.php/near_expired_stock_items").success(function(data){
            ...
            angular.forEach($scope.near_expired_stock_items,function(value){
                ...
                $scope.notification_object.push({
                    'url':'close_to_expiry',
                    'name': $translates('labels.near_expired_item'),
                    'descr':value.vaccine.name,
                    'lot': value.lot_number,
                    'expired_date': value.expiry_date,
                    'type': 'near_expired_item'
                });
                ...
                angular.forEach($scope.items_min_max,function(val){
                    if(value.id == val.item_id){
                        ...
                        if(parseInt(value.amount) > parseInt(val.max_value)){
                            ...
                            $scope.notification_object.push({
                                'url':'above_maximum',
                                'descr':value.itemMinMax.vaccine.name,
                                'max_val': value.itemMinMax.max_value,
                                'type': 'above_maximum_value'
                            });
                            $scope.number_of_notification += 1;
                            ...
                            $scope.notification_object.push({
                                'url':'below_minimum',
                                'name': $translates('labels.item_below_minimum'),
                                'descr':value.itemMinMax.vaccine.name,
                                'amount': value.amount,
                                'min_value': value.itemMinMax.min_value,
                                'type': 'item_below_minimum'
                            });
                            $scope.number_of_notification += 1;
        ...
        $http.get("index.php/expired_stock_items").success(function(data){
            ...
            angular.forEach($scope.stock_items,function(value){
                ...
                $scope.notification_object.push({
                    'url':'expired_items',
                    'name': $translates('labels.expired_item'),
                    'descr':value.vaccine.name,
                    'lot': value.lot_number,
                    'expired_date': value.expiry_date,
                    'type': 'expired_item'
