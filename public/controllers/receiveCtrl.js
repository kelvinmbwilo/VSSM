...
angular.module("vssmApp")
    .controller("receiveCtrl",function ($scope,$http,$mdDialog,$mdToast,$modal,$translate,$filter,$timeout) {
...
//            if($scope.barcode.gtin){$scope.oneItem.packaging_id = $scope.barcode.gtin}
        ...
        $scope.saveArrival = function(item){
            ...
            $http.post("index.php/receive/", item).success(function (d) {
        ...
        $scope.setStoreType = function(id){
            angular.forEach($scope.stores,function(value){
                if(value.id == id){
                    ...
                    $scope.oneItem.used = (value.used_volume/value.net_volume)*1000;
